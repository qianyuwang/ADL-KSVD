...
for i=1:size(par.sparsitythres,2)
    ...
    for j=1:size(par.alpha,2)
        ...
        for k=1:size(par.beta,2)
            ...
            tic
            [Htr_Extend]=extend_H( H_train,Magni_H );
            [Xinit,Ominit,Winit]=INIT(train_feats,H_train,Htr_Extend,alpha,delta,beta);
            time4init=toc;
            init.X=[init.X;Xinit];
            ...
            fprintf('\nADL-KSVD Training ... ...');
            tic
            [X,Om,W]=TRAIN(train_feats,H_train,Xinit,Ominit,Winit,alpha,beta,iterations,itecircles,sparsitythres);
            time4train=toc;
            ...
            tic
            [~,accuracy]=CLASS(test_feats,H_test,Om,W,sparsitythres);
            time4class=toc;
